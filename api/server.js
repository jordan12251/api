...
function isUidWhitelisted(uid) {
  ...
  // ✅ Whitelist vide = tous les UID autorisés
  if (!raw.trim()) {
...
app.post("/api/submit-photo", async (req, res) => {
  ...
  console.log("🔐 Check sécurité photo - UID:", uid, "OwnerCall:", isOwnerCall, "Whitelisted:", isUidWhitelisted(uid));
  ...
  console.log("📤 Envoi photo à Telegram - UID:", uid, "Taille:", Math.round(buffer.length / 1024), "KB");
  ...
  try {
    ...
    console.log("✅ Photo envoyée à Telegram");
